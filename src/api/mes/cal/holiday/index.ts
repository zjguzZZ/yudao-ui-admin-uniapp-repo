...
/** 查询假期设置列表（支持可选日期范围过滤） */
export function getHolidayList(params?: { startDay?: string; endDay?: string }) {
...
/** 保存假期设置（含 upsert 逻辑） */
export function saveHoliday(data: CalHolidayVO) {
  return http.post<CalHolidayVO>(`/mes/cal/holiday/save`, data)
}
