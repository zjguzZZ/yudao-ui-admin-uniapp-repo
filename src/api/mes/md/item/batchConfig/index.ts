...
export function getBatchConfigByItemId(itemId: number) {
  return http.get<MdItemBatchConfigVO>(`/mes/md/item-batch-config/get-by-item-id?itemId=` + itemId)
}

/** 保存批次属性配置（新增或更新） */
export function saveBatchConfig(data: MdItemBatchConfigVO) {
  return http.post<MdItemBatchConfigVO>(`/mes/md/item-batch-config/save`, data)
}
