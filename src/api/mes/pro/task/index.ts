...
/** 新增生产任务 */
export function createTask(data: ProTaskVO) {
  return http.post<number>(`/mes/pro/task/create`, data)
}

/** 修改生产任务 */
export function updateTask(data: ProTaskVO) {
...
export function exportTask(params: any) {
  return http.get<any>(`/mes/pro/task/export-excel`, params)
}

/** 获得甘特图任务列表（非分页） */
export function getGanttTaskList(params: any) {
  return http.get<ProTaskVO[]>(`/mes/pro/task/gantt-list`, params)
}
