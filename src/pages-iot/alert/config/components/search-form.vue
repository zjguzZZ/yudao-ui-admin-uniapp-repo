  ...
  <wd-popup v-model="visible" position="top" :custom-style="getTopPopupStyle()" :modal-style="getTopPopupModalStyle()" @close="visible = false">
    <view class="yd-search-form-container">
      <view class="yd-search-form-item">
        <view class="yd-search-form-label">配置名称</view>
        <wd-input v-model="formData.name" placeholder="请输入配置名称" clearable />
      </view>
      <view class="yd-search-form-item">
        <view class="yd-search-form-label">配置状态</view>
...
const formData = reactive({ name: undefined as string | undefined, status: -1 }) // 搜索表单数据
const placeholder = computed(() => formData.name ? '配置名称:' + formData.name : '搜索告警配置')

/** 搜索按钮操作 */
function handleSearch() { visible.value = false; emit('search', { name: formData.name, status: formData.status === -1 ? undefined : formData.status }) }

/** 重置按钮操作 */
function handleReset() { formData.name = undefined; formData.status = -1; visible.value = false; emit('reset') }
</script>
