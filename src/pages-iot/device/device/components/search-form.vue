  ...
  <wd-popup v-model="visible" position="top" :custom-style="getTopPopupStyle()" :modal-style="getTopPopupModalStyle()" @close="visible = false">
    <view class="yd-search-form-container">
      ...
      <view class="yd-search-form-item">
        ...
        <wd-radio-group v-model="formData.state" type="button">
...
const formData = reactive({ deviceName: undefined as string | undefined, nickname: undefined as string | undefined, state: -1 }) // 搜索表单数据
const placeholder = computed(() => formData.deviceName || formData.nickname ? '搜索条件已设置' : '搜索设备')
...
function handleSearch() {
  ...
  emit('search', {
    ...
    state: formData.state === -1 ? undefined : formData.state,
  })
}
