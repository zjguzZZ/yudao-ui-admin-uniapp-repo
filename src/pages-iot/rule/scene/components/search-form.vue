  ...
  <wd-popup v-model="visible" position="top" :custom-style="getTopPopupStyle()" :modal-style="getTopPopupModalStyle()" @close="visible = false">
    <view class="yd-search-form-container">
      <view class="yd-search-form-item">
        <view class="yd-search-form-label">规则名称</view>
        <wd-input v-model="formData.name" placeholder="请输入规则名称" clearable />
      </view>
      <view class="yd-search-form-item">
        <view class="yd-search-form-label">规则状态</view>
...
const placeholder = computed(() => formData.name ? '规则名称:' + formData.name : '搜索场景联动')

/** 搜索按钮操作 */
function handleSearch() {
  visible.value = false
  emit('search', { name: formData.name, status: formData.status === -1 ? undefined : formData.status })
}
