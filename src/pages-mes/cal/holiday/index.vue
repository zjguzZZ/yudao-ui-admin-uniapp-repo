        ...
        >
          <view class="p-24rpx">
            ...
            <view class="mb-12rpx flex items-center text-28rpx text-[#666]">
              <text class="mr-8rpx shrink-0 text-[#999]">type：</text>
              <text class="min-w-0 flex-1 truncate">{{ formatFieldValue(item.type) || '-' }}</text>
            </view>
            <view class="mb-12rpx flex items-center text-28rpx text-[#666]">
              <text class="mr-8rpx shrink-0 text-[#999]">remark：</text>
...
async function queryList(pageNo: number, pageSize: number) {
  try {
    ...
    const data = await getHolidayList(params as any)
...
function handleAdd() {
  uni.navigateTo({
    url: '/pages-mes/cal/holiday/form/index',
  })
}
...
function handleDetail(item: any) {
  uni.navigateTo({
    url: `/pages-mes/cal/holiday/detail/index?id=${(item as any).id}`,
  })
}

/** 初始化 */
onMounted(() => {
  uni.$on('mes:cal:holiday:reload', reload)
})

/** 卸载 */
onUnload(() => {
  uni.$off('mes:cal:holiday:reload', reload)
})
</script>
