  ...
  <view class="yd-page-container">
    ...
    <view>
      <wd-cell-group border>
        ...
        <wd-cell title="保养时间" :value="formatFieldValue(formData?.maintenTime) || '-'" />
        <wd-cell title="保养人" :value="formatFieldValue(formData?.nickname) || '-'" />
        <wd-cell title="状态" :value="formatFieldValue(formData?.status) || '-'" />
        <wd-cell title="编号" :value="formatFieldValue(formData?.id) || '-'" />
        <wd-cell title="计划编号" :value="formatFieldValue(formData?.planId) || '-'" />
        <wd-cell title="设备编号" :value="formatFieldValue(formData?.machineryId) || '-'" />
        <wd-cell title="用户编号" :value="formatFieldValue(formData?.userId) || '-'" />
    ...
    <view class="yd-detail-footer">
      <view class="yd-detail-footer-actions">
        <wd-button
          v-if="hasAccessByCodes(['mes:dv-mainten-record:update'])"
          class="flex-1" type="warning" @click="handleEdit"
        >
          编辑
        </wd-button>
        <wd-button
          v-if="hasAccessByCodes(['mes:dv-mainten-record:delete'])"
...
import { getMaintenRecord, deleteMaintenRecord } from '@/api/mes/dv/maintenrecord'
...
async function getDetail() {
  ...
  try {
    toast.loading('加载中...')
    formData.value = await getMaintenRecord(props.id)
  } finally {
    toast.close()
  }
}
...
async function handleDelete() {
  ...
  deleting.value = true
  try {
    await deleteMaintenRecord(props.id)
    toast.success('删除成功')
