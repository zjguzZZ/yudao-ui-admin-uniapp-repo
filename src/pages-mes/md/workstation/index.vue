      ...
      <view class="p-24rpx">
        ...
        >
          <view class="p-24rpx">
            ...
            <view class="mb-12rpx flex items-center text-28rpx text-[#666]">
              <text class="mr-8rpx shrink-0 text-[#999]">工作站地点：</text>
              <text class="min-w-0 flex-1 truncate">{{ formatFieldValue(item.address) || '-' }}</text>
            </view>
            <view class="mb-12rpx flex items-center text-28rpx text-[#666]">
              <text class="mr-8rpx shrink-0 text-[#999]">所在车间：</text>
...
async function queryList(pageNo: number, pageSize: number) {
  try {
    ...
    const data = await getWorkstationPage(params as any)
...
function handleAdd() {
  uni.navigateTo({
    url: '/pages-mes/md/workstation/form/index',
  })
}
...
function handleDetail(item: any) {
  uni.navigateTo({
    url: `/pages-mes/md/workstation/detail/index?id=${(item as any).id}`,
  })
}
