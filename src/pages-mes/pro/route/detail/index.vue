  ...
  <view class="yd-page-container">
    ...
    <view>
      <wd-cell-group border>
        <wd-cell title="路线编码" :value="formatFieldValue(formData?.code) || '-'" />
        <wd-cell title="路线名称" :value="formatFieldValue(formData?.name) || '-'" />
        <wd-cell title="路线说明" :value="formatFieldValue(formData?.description) || '-'" />
    ...
    <view class="yd-detail-footer">
      <view class="yd-detail-footer-actions">
        <wd-button
          v-if="hasAccessByCodes(['mes:pro-route:update'])"
          class="flex-1" type="warning" @click="handleEdit"
        >
          编辑
        </wd-button>
        <wd-button
          v-if="hasAccessByCodes(['mes:pro-route:delete'])"
...
import { getRoute, deleteRoute } from '@/api/mes/pro/route'
...
function handleEdit() {
  uni.navigateTo({
    url: `/pages-mes/pro/route/form/index?id=${props.id}`,
  })
}
