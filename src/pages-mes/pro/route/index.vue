<template>
  <view class="yd-page-container yd-page-container-paging">
        ...
        >
          <view class="p-24rpx">
            ...
            <view class="mb-12rpx flex items-center text-28rpx text-[#666]">
              <text class="mr-8rpx shrink-0 text-[#999]">路线名称：</text>
              <text class="min-w-0 flex-1 truncate">{{ formatFieldValue(item.name) || '-' }}</text>
            </view>
            <view class="mb-12rpx flex items-center text-28rpx text-[#666]">
              <text class="mr-8rpx shrink-0 text-[#999]">路线说明：</text>
    ...
    <!-- 新增按钮 -->
    <wd-fab
      v-if="hasAccessByCodes(['mes:pro-route:create'])"
...
async function queryList(pageNo: number, pageSize: number) {
  try {
    ...
    const data = await getRoutePage(params as any)
...
function handleAdd() {
  uni.navigateTo({
    url: '/pages-mes/pro/route/form/index',
  })
}
...
function handleDetail(item: any) {
  uni.navigateTo({
    url: `/pages-mes/pro/route/detail/index?id=${(item as any).id}`,
  })
}
