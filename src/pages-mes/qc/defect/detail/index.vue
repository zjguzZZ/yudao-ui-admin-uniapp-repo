  ...
  <view class="yd-page-container">
    ...
    <view>
      <wd-cell-group border>
        <wd-cell title="缺陷编码" :value="formatFieldValue(formData?.code) || '-'" />
        <wd-cell title="缺陷描述" :value="formatFieldValue(formData?.name) || '-'" />
        <wd-cell title="检测项类型" :value="formatFieldValue(formData?.type) || '-'" />
        <wd-cell title="缺陷等级" :value="formatFieldValue(formData?.level) || '-'" />
...
import { getDefect, deleteDefect } from '@/api/mes/qc/defect'
...
function handleEdit() {
  uni.navigateTo({
    url: `/pages-mes/qc/defect/form/index?id=${props.id}`,
  })
}
