<template>
  <view class="yd-page-container yd-page-container-paging">
    ...
    <z-paging
      ...
      empty-view-text="暂无来料检验单（IQC）数据"
      @query="queryList"
    >
      <view class="p-24rpx">
        <view
        ...
        >
          <view class="p-24rpx">
            ...
            <view class="mb-12rpx flex items-center text-28rpx text-[#666]">
              <text class="mr-8rpx shrink-0 text-[#999]">来料检验单名称：</text>
              <text class="min-w-0 flex-1 truncate">{{ formatFieldValue(item.name) || '-' }}</text>
            </view>
            <view class="mb-12rpx flex items-center text-28rpx text-[#666]">
              <text class="mr-8rpx shrink-0 text-[#999]">供应商简称：</text>
              <text class="min-w-0 flex-1 truncate">{{ formatFieldValue(item.vendorNickname) || '-' }}</text>
            </view>
            <view class="mb-12rpx flex items-center text-28rpx text-[#666]">
              <text class="mr-8rpx shrink-0 text-[#999]">供应商批次号：</text>
              <text class="min-w-0 flex-1 truncate">{{ formatFieldValue(item.vendorBatch) || '-' }}</text>
            </view>
            <view class="mb-12rpx flex items-center text-28rpx text-[#666]">
            ...
            <view class="mb-12rpx flex items-center text-28rpx text-[#666]">
              <text class="mr-8rpx shrink-0 text-[#999]">接收数量：</text>
              <text class="min-w-0 flex-1 truncate">{{ formatFieldValue(item.receivedQuantity) || '-' }}</text>
            </view>
          </view>
        </view>
      </view>
...
async function queryList(pageNo: number, pageSize: number) {
  try {
    ...
    const data = await getIqcPage(params as any)
...
function handleDetail(item: any) {
  uni.navigateTo({
    url: `/pages-mes/qc/iqc/detail/index?id=${(item as any).id}`,
  })
}
