<template>
  <view class="yd-page-container yd-page-container-paging">
    ...
    <z-paging
      ...
      empty-view-text="暂无出货检验单（OQC）数据"
      @query="queryList"
    >
      <view class="p-24rpx">
        <view
        ...
        >
          <view class="p-24rpx">
            ...
            <view class="mb-12rpx flex items-center text-28rpx text-[#666]">
              ...
              <text class="min-w-0 flex-1 truncate">{{ formatFieldValue(item.clientNickname) || '-' }}</text>
            </view>
            <view class="mb-12rpx flex items-center text-28rpx text-[#666]">
...
async function queryList(pageNo: number, pageSize: number) {
  try {
    ...
    const data = await getOqcPage(params as any)
...
function handleDetail(item: any) {
  uni.navigateTo({
    url: `/pages-mes/qc/oqc/detail/index?id=${(item as any).id}`,
  })
}
