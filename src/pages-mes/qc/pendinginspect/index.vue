          ...
          <view class="p-24rpx">
            <view class="mb-16rpx flex items-center justify-between gap-16rpx">
              <view class="min-w-0 flex-1 truncate text-32rpx text-[#333] font-semibold">
                {{ formatFieldValue(item.sourceDocCode) || '-' }}
              </view>
            ...
            <view class="mb-12rpx flex items-center text-28rpx text-[#666]">
              ...
              <text class="min-w-0 flex-1 truncate">{{ formatFieldValue(item.qcType) || '-' }}</text>
            </view>
            <view class="mb-12rpx flex items-center text-28rpx text-[#666]">
...
import type { QcPendingInspectVO } from '@/api/mes/qc/pendinginspect'
...
async function queryList(pageNo: number, pageSize: number) {
  try {
    ...
    const data = await getPendingInspectPage(params as any)
