<template>
  <view class="yd-page-container">
    <!-- 顶部导航栏 -->
    <wd-navbar
      :title="getTitle"
      left-arrow placeholder safe-area-inset-top fixed
      @click-left="handleBack"
    />

    <!-- 表单区域 -->
    <view>
      <wd-form ref="formRef" :model="formData" :schema="formSchema">
        <wd-cell-group border>
          <wd-form-item title="configId" title-width="200rpx" prop="configId" center>
            <wd-input-number v-model="formData.configId" :min="0" />
          </wd-form-item>
          <wd-form-item title="format" title-width="200rpx" prop="format" center>
            <wd-input-number v-model="formData.format" :min="0" />
          </wd-form-item>
          <wd-form-item title="bizType" title-width="200rpx" prop="bizType" center>
            <wd-input-number v-model="formData.bizType" :min="0" />
          </wd-form-item>
          <wd-form-item title="content" title-width="200rpx" prop="content">
            <wd-textarea
              v-model="formData.content"
              placeholder="请输入content"
              :maxlength="200"
              show-word-limit
              clearable
            />
          </wd-form-item>
          <wd-form-item title="bizId" title-width="200rpx" prop="bizId" center>
            <wd-input-number v-model="formData.bizId" :min="0" />
          </wd-form-item>
          <wd-form-item title="bizCode" title-width="200rpx" prop="bizCode">
            <wd-input
              v-model="formData.bizCode"
              clearable
              placeholder="请输入bizCode"
            />
          </wd-form-item>
          <wd-form-item title="bizName" title-width="200rpx" prop="bizName">
            <wd-input
              v-model="formData.bizName"
              clearable
              placeholder="请输入bizName"
            />
          </wd-form-item>
          <wd-form-item title="status" title-width="200rpx" prop="status" center>
            <wd-input-number v-model="formData.status" :min="0" />
          </wd-form-item>
          <wd-form-item title="remark" title-width="200rpx" prop="remark">
            <wd-textarea
              v-model="formData.remark"
              placeholder="请输入remark"
              :maxlength="200"
              show-word-limit
              clearable
            />
          </wd-form-item>
        </wd-cell-group>
      </wd-form>
    </view>

    <!-- 底部保存按钮 -->
    <view class="yd-detail-footer">
      <wd-button type="primary" block :loading="formLoading" @click="handleSubmit">
        保存
      </wd-button>
    </view>
  </view>
</template>

<script lang="ts" setup>
import type { FormInstance } from '@wot-ui/ui/components/wd-form/types'
import type { WmBarcodeVO } from '@/api/mes/wm/barcode'
import { useToast } from '@wot-ui/ui/components/wd-toast'
import { computed, onMounted, ref } from 'vue'
import { createBarcode, updateBarcode, getBarcode } from '@/api/mes/wm/barcode'
import { navigateBackPlus } from '@/utils'
import { createFormSchema } from '@/utils/wot'

const props = defineProps<{
  id?: number | string | any
}>()

definePage({
  style: {
    navigationBarTitleText: '',
    navigationStyle: 'custom',
  },
})

const toast = useToast()
const getTitle = computed(() => props.id ? '编辑条码' : '新增条码')
const formLoading = ref(false) // 表单提交状态
const formData = ref<any>({
  id: undefined,
  configId: undefined,
  format: undefined,
  bizType: undefined,
  content: '',
  bizId: undefined,
  bizCode: '',
  bizName: '',
  status: undefined,
  remark: '',
} as WmBarcodeVO) // 表单数据
const formSchema = createFormSchema({

})
const formRef = ref<FormInstance>() // 表单组件引用

/** 返回上一页 */
function handleBack() {
  navigateBackPlus('/pages-mes/wm/barcode/index')
}

/** 加载详情 */
async function getDetail() {
  if (!props.id) {
    return
  }
  formData.value = await getBarcode(props.id)
}

/** 提交表单 */
async function handleSubmit() {
  const result = await formRef.value?.validate()
  if (result && !result.valid) {
    return
  }

  formLoading.value = true
  try {
    if (props.id) {
      await updateBarcode(formData.value)
      toast.success('修改成功')
    } else {
      await createBarcode(formData.value)
      toast.success('新增成功')
    }
    uni.$emit('mes:wm:barcode:reload')
    setTimeout(() => {
      handleBack()
    }, 500)
  } finally {
    formLoading.value = false
  }
}

/** 初始化 */
onMounted(() => {
  getDetail()
})
</script>

<style lang="scss" scoped>
</style>
