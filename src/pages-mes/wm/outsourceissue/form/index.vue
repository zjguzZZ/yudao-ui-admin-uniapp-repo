    ...
    <view>
      <wd-form ref="formRef" :model="formData" :schema="formSchema">
        <wd-cell-group border>
          ...
          <wd-form-item title="workorderName" title-width="200rpx" prop="workorderName">
            <wd-input
              v-model="formData.workorderName"
              clearable
              placeholder="请输入workorderName"
            />
          </wd-form-item>
          <wd-datetime-picker
...
import { createOutsourceIssue, updateOutsourceIssue, getOutsourceIssue } from '@/api/mes/wm/outsourceissue'
...
const getTitle = computed(() => props.id ? '编辑外协发料' : '新增外协发料')
...
async function handleSubmit() {
  ...
  try {
    if (props.id) {
      ...
      await createOutsourceIssue(formData.value)
      toast.success('新增成功')
    }
