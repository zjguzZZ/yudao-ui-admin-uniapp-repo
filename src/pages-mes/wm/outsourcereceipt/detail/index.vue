  ...
  <view class="yd-page-container">
    ...
    <view>
      <wd-cell-group border>
        ...
        <wd-cell title="外协工单号" :value="formatFieldValue(formData?.workOrderCode) || '-'" />
    ...
    <view class="yd-detail-footer">
      <view class="yd-detail-footer-actions">
        <wd-button
          v-if="hasAccessByCodes(['mes:wm-outsource-receipt:update'])"
          class="flex-1" type="warning" @click="handleEdit"
        >
          编辑
        </wd-button>
        <wd-button
          v-if="hasAccessByCodes(['mes:wm-outsource-receipt:delete'])"
...
import { getOutsourceReceipt, deleteOutsourceReceipt } from '@/api/mes/wm/outsourcereceipt'
...
function handleEdit() {
  uni.navigateTo({
    url: `/pages-mes/wm/outsourcereceipt/form/index?id=${props.id}`,
  })
}
...
async function handleDelete() {
  ...
  deleting.value = true
  try {
    await deleteOutsourceReceipt(props.id)
    toast.success('删除成功')
