  ...
  <view class="yd-page-container">
    ...
    <view>
      <wd-cell-group border>
        <wd-cell title="领料单编号" :value="formatFieldValue(formData?.code) || '-'" />
        <wd-cell title="领料单名称" :value="formatFieldValue(formData?.name) || '-'" />
        ...
        <wd-cell title="客户编号" :value="formatFieldValue(formData?.clientCode) || '-'" />
        <wd-cell title="客户名称" :value="formatFieldValue(formData?.clientName) || '-'" />
        <wd-cell title="需求时间" :value="formatFieldValue(formData?.requiredTime) || '-'" />
        ...
        <wd-cell title="workstationCode" :value="formatFieldValue(formData?.workstationCode) || '-'" />
    ...
    <view class="yd-detail-footer">
      <view class="yd-detail-footer-actions">
        <wd-button
          v-if="hasAccessByCodes(['mes:wm-product-issue:update'])"
          class="flex-1" type="warning" @click="handleEdit"
        >
          编辑
        </wd-button>
        <wd-button
          v-if="hasAccessByCodes(['mes:wm-product-issue:delete'])"
...
import { getProductIssue, deleteProductIssue } from '@/api/mes/wm/productissue'
...
async function getDetail() {
  ...
  try {
    toast.loading('加载中...')
    formData.value = await getProductIssue(props.id)
  } finally {
    toast.close()
  }
}
...
function handleEdit() {
  uni.navigateTo({
    url: `/pages-mes/wm/productissue/form/index?id=${props.id}`,
  })
}
...
async function handleDelete() {
  ...
  deleting.value = true
  try {
    await deleteProductIssue(props.id)
    toast.success('删除成功')
