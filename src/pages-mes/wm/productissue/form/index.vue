    ...
    <view>
      <wd-form ref="formRef" :model="formData" :schema="formSchema">
        <wd-cell-group border>
          ...
          <wd-form-item title="workstationCode" title-width="200rpx" prop="workstationCode">
            <wd-input
              v-model="formData.workstationCode"
              clearable
              placeholder="请输入workstationCode"
            />
          </wd-form-item>
...
import { createProductIssue, updateProductIssue, getProductIssue } from '@/api/mes/wm/productissue'
...
const getTitle = computed(() => props.id ? '编辑生产领料' : '新增生产领料')
...
async function handleSubmit() {
  ...
  try {
    if (props.id) {
      ...
      await createProductIssue(formData.value)
      toast.success('新增成功')
    }
