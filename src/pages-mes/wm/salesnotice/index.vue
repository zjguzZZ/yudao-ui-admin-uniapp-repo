        ...
        >
          <view class="p-24rpx">
            ...
            <view class="mb-12rpx flex items-center text-28rpx text-[#666]">
              ...
              <text class="min-w-0 flex-1 truncate">{{ formatFieldValue(item.salesDate) || '-' }}</text>
            </view>
            <view class="mb-12rpx flex items-center text-28rpx text-[#666]">
              <text class="mr-8rpx shrink-0 text-[#999]">收货人：</text>
              <text class="min-w-0 flex-1 truncate">{{ formatFieldValue(item.recipientName) || '-' }}</text>
            </view>
            <view class="mb-12rpx flex items-center text-28rpx text-[#666]">
              <text class="mr-8rpx shrink-0 text-[#999]">联系方式：</text>
              <text class="min-w-0 flex-1 truncate">{{ formatFieldValue(item.recipientTelephone) || '-' }}</text>
            </view>
          </view>
        </view>
      </view>
...
async function queryList(pageNo: number, pageSize: number) {
  try {
    ...
    const data = await getSalesNoticePage(params as any)
...
function handleDetail(item: any) {
  uni.navigateTo({
    url: `/pages-mes/wm/salesnotice/detail/index?id=${(item as any).id}`,
  })
}
