      ...
      <view class="p-24rpx">
        ...
        >
          <view class="p-24rpx">
            ...
            <view class="mb-12rpx flex items-center text-28rpx text-[#666]">
              <text class="mr-8rpx shrink-0 text-[#999]">SN 码数量：</text>
              <text class="min-w-0 flex-1 truncate">{{ formatFieldValue(item.count) || '-' }}</text>
            </view>
            <view class="mb-12rpx flex items-center text-28rpx text-[#666]">
...
async function queryList(pageNo: number, pageSize: number) {
  try {
    ...
    const data = await getSnGroupPage(params as any)
...
function handleDetail(item: any) {
  uni.navigateTo({
    url: `/pages-mes/wm/sn/detail/index?id=${(item as any).uuid}`,
  })
}
